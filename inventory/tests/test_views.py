...
from inventory.views import apply_product_filters
...
class DashboardViewTests(TestCase):
    ...
    def test_staff_sees_cost_and_admin_columns(self):
        self.client.force_login(self.user)
        response = self.client.get(reverse("dashboard"))
...
class SearchProductsViewTests(TestCase):
    ...
    def test_staff_gets_cost(self):
        self.client.force_login(self.user)
        response = self.client.get(reverse("search_products"))
        data = json.loads(response.content)
        self.assertEqual(data["products"][0]["cost"], "50.00")
...
class ManualSyncViewTests(TestCase):
    ...
    def test_success_redirects_with_message(self, mock_call):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse("sync_page"))
...
class SyncPageViewTests(TestCase):
    ...
    def test_requires_admin(self):
        response = self.client.get(reverse("sync_page"))
        self.assertEqual(response.status_code, 302)
        self.client.force_login(self.nonstaff)
        response = self.client.get(reverse("sync_page"))
        self.assertEqual(response.status_code, 302)
...
class SyncProductsViewTests(TestCase):
    ...
    def test_requires_admin(self):
        self.client.force_login(self.user)
        response = self.client.get(reverse("sync_products"))
        self.assertEqual(response.status_code, 302)

    @patch("inventory.views.call_command")
    def test_runs_inventory_sync_and_redirects(self, mock_call):
        self.client.force_login(self.admin)
        response = self.client.get(reverse("sync_products"))
...
class SyncCustomersViewTests(TestCase):
    ...
    def test_requires_admin(self):
        self.client.force_login(self.user)
        response = self.client.get(reverse("sync_customers"))
        self.assertEqual(response.status_code, 302)

    @patch("inventory.views.call_command")
    def test_runs_customer_sync_and_redirects(self, mock_call):
        self.client.force_login(self.admin)
        response = self.client.get(reverse("sync_customers"))
        mock_call.assert_called_once_with("sync_customers")
        self.assertRedirects(response, reverse("sync_page"))


class ApplyProductFiltersTests(TestCase):
    ...
    def test_search_matches_name(self):
        result = apply_product_filters(
            Product.objects.all(), search="Drill"
        )
        self.assertEqual(list(result), [self.product])

    def test_category_filter(self):
        result = apply_product_filters(
            Product.objects.all(), category="Garden"
        )
        self.assertEqual(list(result), [self.other])

    def test_subcategory_filter(self):
        result = apply_product_filters(
            Product.objects.all(), subcategory="Power"
        )
        self.assertEqual(list(result), [self.product])

    def test_level3_and_level4_filters(self):
        result = apply_product_filters(
            Product.objects.all(), level3="Drills", level4="Electric"
        )
        self.assertEqual(list(result), [self.product])

    def test_combined_search_and_category(self):
        result = apply_product_filters(
            Product.objects.all(), search="Shovel", category="Tools"
        )
        self.assertEqual(list(result), [])


class OfflineProductsFieldsTests(TestCase):
    ...
    def test_returns_full_hierarchy_fields(self):
        self.client.force_login(self.user)
        response = self.client.get(reverse("offline_products"))
        data = json.loads(response.content)
        row = next(r for r in data if r["item"] == self.product.item)
        self.assertEqual(row["category"], "Tools")
        self.assertEqual(row["subcategory"], "Power")
        self.assertEqual(row["level3"], "Drills")
        self.assertEqual(row["level4"], "Electric")
        self.assertEqual(row["hierarchy_path"], "Tools > Power > Drills > Electric")
    ...
    def test_excludes_inactive_products(self):
        ...
        self.assertNotIn(self.inactive.item, items)
